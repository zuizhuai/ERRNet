...
def Train():
    ...
    parse.add_argument('--apex', default=True, type=bool)
    ...
    model = ERRNet(backbone_pre=args.backbone_pred).cuda()
    ...
    try:
        for epoch in range(args.start_epoch + 1, args.start_epoch + 1 + args.epoch):
            ...
            if epoch % args.save_epoch == 0:
                torch.save(model.state_dict(), args.save_path + 'ERRNet_epoch_%d.pth' % (epoch + 1))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(args.save_path):
            os.makedirs(args.save_path)
        torch.save(model.state_dict(), args.save_path + '\\' + 'ERRNet_epoch_{}.pth'.format(epoch + 1))
